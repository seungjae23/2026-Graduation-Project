...
class _GuesserGameScreenState extends State<GuesserGameScreen> {
  ...
  Future<void> _submitAnswer(String actualCorrectAnswer) async {
    ...
    final isCorrect = answer.replaceAll(RegExp(r'\s+'), '').toLowerCase() == actualCorrectAnswer.replaceAll(RegExp(r'\s+'), '').toLowerCase();
  ...
  void _moveToNextRound(int nextScore) {
    ...
    if (currentRound >= totalRound) {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => ResultScreen(firstPlayerName: widget.firstPlayerName, secondPlayerName: widget.secondPlayerName, firstPlayerScore: updatedFirstPlayerScore, secondPlayerScore: updatedSecondPlayerScore)));
      return;
    }
